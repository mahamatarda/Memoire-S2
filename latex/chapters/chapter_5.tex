...
\textbf{Le coût computationnel}~: un premier essai de Random Forest sans contraintes de profondeur ni parallélisation a conduit à un entraînement de plus de 3 heures interrompu avant convergence. Cela m'a forcé à comprendre les paramètres internes (\texttt{max\_depth}, \texttt{n\_jobs}). Le SVM a été écarté pour la même raison~: complexité $\mathcal{O}(n^2)$ incompatible avec 100~000 observations.
...
\begin{itemize}
  ...
  \item \textbf{Gestion du déséquilibre de classes}~: comparaison concrète entre ajustement des poids (\texttt{class\_weight}) et SMOTE~\cite{chawla2002}, et maîtrise de la différence entre accuracy, F1-score et AUC-ROC~\cite{sokolova2009}.
